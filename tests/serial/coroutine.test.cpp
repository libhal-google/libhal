...
boost::ut::suite serial_read_uint32_test = []() {
  ...
  const std::array<hal::byte, 5> read_out{ '1', '2', '3', '4', ',' };
  const std::array<hal::byte, 5> read_out_no_ints{ 'a', 'b', 'c', 'd', 'e' };
  const std::array<hal::byte, 5> read_out_internal_int{
    'a', 'b', '5', '1', 'e'
  };

  "[success] read_uint32 one shot"_test = [=]() {
    // Setup
    fake_serial serial;
    serial.m_read_function =
      [&read_out, count = size_t(0)](
    ...
    auto integer_reader = read_uint32(serial);
    std::array<work_state, 5> actual;

    // Exercise
    actual[0] = integer_reader().value();
    actual[1] = integer_reader().value();
    actual[2] = integer_reader().value();
    actual[3] = integer_reader().value();
    actual[4] = integer_reader().value();
    ...
    expect(that % work_state::finished == actual[3]);
    expect(that % work_state::finished == actual[4]);
    expect(that % 1234 == integer_reader.get().value());
  };

  "[success] read_uint32 one shot no integers"_test = [=]() {
    // Setup
    fake_serial serial;
    serial.m_read_function =
      [&read_out_no_ints, count = size_t(0)](
        std::span<hal::byte> p_data) mutable -> result<serial::read_t> {
      if (count < read_out_no_ints.size()) {
    ...
    expect(that % work_state::in_progress == actual[2]);
    expect(that % work_state::in_progress == actual[3]);
    expect(that % work_state::in_progress == actual[4]);
    expect(that % false == integer_reader.get().has_value());
  };

  "[success] read_uint32 one byte at a time"_test = [=]() {
    // Setup
    fake_serial serial;
    serial.m_read_function =
      [&read_out, count = size_t(0), send_byte = false](
  ...
  "[success] read_uint32 one shot internal integer one byte at at time"_test =
    [=]() {
      // Setup
      fake_serial serial;
      serial.m_read_function =
        [&read_out_internal_int, count = size_t(0)](
          std::span<hal::byte> p_data) mutable -> result<serial::read_t> {
        if (count < read_out_internal_int.size()) {
      ...
      auto integer_reader = read_uint32(serial, 1);
      std::array<work_state, 5> actual;
      ...
      expect(that % work_state::in_progress == actual[3]);
      expect(that % work_state::finished == actual[4]);
      expect(that % 51 == integer_reader.get().value());
    };

  "[success] read_uint32 in two blocks"_test = [=]() {
    // Setup
    fake_serial serial;
    serial.m_read_function =
      [&read_out, count = size_t(0)](
  ...
  "[success] read_uint32 return control after 1 byte"_test = [=]() {
    // Setup
    fake_serial serial;
    serial.m_read_function =
      [&read_out, count = size_t(0)](
  ...
  "[failure] read_uint32 return error on read"_test = [=]() {
    // Setup
    fake_serial serial;
    serial.m_read_function =
      []([[maybe_unused]] std::span<hal::byte> p_data) mutable
  ...
  "[success] read_uint32 error every other byte"_test = [=]() {
    // Setup
    fake_serial serial;
    serial.m_read_function =
      [&read_out, count = size_t(0), send_byte = false](
    ...
    // Exercise
    auto actual0 = integer_reader();
    auto actual1 = integer_reader();
    auto actual2 = integer_reader();
    auto actual3 = integer_reader();
    auto actual4 = integer_reader();
    auto actual5 = integer_reader();
    auto actual6 = integer_reader();
    auto actual7 = integer_reader();
    auto actual8 = integer_reader();
    auto actual9 = integer_reader();
    auto actual10 = integer_reader();
  ...
  "[failure] read_uint32::get() without reads"_test = [=]() {
    // Setup
    fake_serial serial;
