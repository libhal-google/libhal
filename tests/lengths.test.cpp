...
void lengths_test() 
{
    ...
    "Lengths_Type UDL_Creation Test"_test = []() {
        meters micrometer = 1.0_um, millimeter = 1.0_mm, meter = 1.0_m, kilometer = 1.0_km,
               inch = 1.0_inch, yard = 1.0_yards, mile = 1.0_miles;

        expect(that % compare_floats(micrometer, static_cast<float>(1.0f / std::micro::den)));
        expect(that % compare_floats(millimeter, static_cast<float>(1.0f / std::milli::den)));
        expect(that % compare_floats(meter, 1.0f));
        expect(that % compare_floats(kilometer, static_cast<float>(1.0f * std::kilo::num)));
