...
boost::ut::suite frequency_user_defined_literals_test = []() {
  ...
  "frequency"_test = []() {
    ...
    "duty_cycle"_test = []() {
      ...
      expect(that % 1.00_d == static_cast<double>(duty_cycle{ 2800, 0 }));
      expect(that % 0.80_d == static_cast<double>(duty_cycle{ 2240, 560 }));
      expect(that % 0.55_d == static_cast<double>(duty_cycle{ 1540, 1260 }));
      expect(that % 0.33_d == static_cast<double>(duty_cycle{ 924, 1876 }));
      expect(that % 0.12_d == static_cast<double>(duty_cycle{ 336, 2464 }));
      expect(that % 0.00_d == static_cast<double>(duty_cycle{ 0, 2800 }));
      expect(that % 0.50_d == static_cast<double>(duty_cycle{
                                std::numeric_limits<std::int32_t>::max(),
                                std::numeric_limits<std::int32_t>::max() }));
    };
