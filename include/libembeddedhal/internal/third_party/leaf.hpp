...
static_assert(BOOST_LEAF_TLS_ARRAY_SIZE <=
                configNUM_THREAD_LOCAL_STORAGE_POINTERS,
              "Invalid BOOST_LEAF_TLS_ARRAY_SIZE");
...
static_assert(BOOST_LEAF_TLS_ARRAY_SIZE > 0, "Bad BOOST_LEAF_TLS_ARRAY_SIZE");
...
BOOST_LEAF_TLS_INDEX_TYPE index_counter<T>::c =
  BOOST_LEAF_TLS_ARRAY_START_INDEX;
...
{
  using type =
    mp_to_bool<std::is_base_of<mp_identity<V>, mp_inherit<mp_identity<T>...>>>;
};
...
{};
template<class C, class R, class... A>
struct function_traits_impl<R (C::*)(A...)> : function_traits_impl<R(C&, A...)>
{};
template<class C, class R, class... A>
...
template<class T>
struct has_printable_member_value<T,
                                  decltype(std::declval<std::ostream&>()
                                             << std::declval<T const&>().value,
                                           void())> : std::true_type
{};
...
#define BOOST_LEAF_ASSIGN(v, r)                                                \
  ...
  static_assert(::boost::leaf::is_result_type<                                 \
                ...
                "BOOST_LEAF_ASSIGN/BOOST_LEAF_AUTO requires a result object "  \
                "as the second argument (see is_result_type)");                \
  if (!BOOST_LEAF_TMP)                                                         \
...
namespace boost {
BOOST_LEAF_NORETURN void throw_exception(
  std::exception const&);  // user defined
}
...
public:
  ...
  BOOST_LEAF_CONSTEXPR explicit e_unexpected_count(
...
public:
  ...
  BOOST_LEAF_CONSTEXPR error_id load(Item&&... item) const noexcept
  {
    if (int err_id = value()) {
      int const unused[] = { 42,
                             leaf_detail::load_item<Item>::load(
...
#define BOOST_LEAF_EXCEPTION                                                   \
  ::boost::leaf::leaf_detail::inject_loc{ __FILE__, __LINE__, __FUNCTION__ } + \
    ::boost::leaf::exception
#define BOOST_LEAF_THROW_EXCEPTION                                             \
  ::boost::leaf::leaf_detail::throw_with_loc{ __FILE__,                        \
                                              __LINE__,                        \
                                              __FUNCTION__ } +                 \
    ::boost::leaf::exception
...
inline typename std::enable_if<
  ...
  leaf_detail::exception<typename std::remove_reference<Ex>::type>>::type
exception(error_id err, Ex&& ex, E&&... e) noexcept
{
  static_assert(
    !leaf_detail::at_least_one_derives_from_std_exception<E...>::value,
    "Error objects passed to leaf::exception may not derive from "
    "std::exception");
  return leaf_detail::exception<typename std::remove_reference<Ex>::type>(
...
inline typename std::enable_if<
  ...
  leaf_detail::exception<std::exception>>::type
exception(error_id err, E1&& car, E&&... cdr) noexcept
{
  static_assert(
    ...
    "Error objects passed to leaf::exception may not derive from "
    "std::exception");
  return leaf_detail::exception<std::exception>(
...
public:
  ...
  {
    BOOST_LEAF_ASSERT(ctx_->captured_id_);
    tls::write_uint32<tls_tag_id_factory_current_id>(
      ctx_->captured_id_.value());
...
struct tuple_for_each
{
  BOOST_LEAF_CONSTEXPR static void activate(Tuple& tup) noexcept
  {
    static_assert(
      !std::is_same<
        error_info,
        typename std::decay<decltype(std::get<I - 1>(tup))>::type>::value,
...
inline context_ptr make_shared_context() noexcept
{
  return std::make_shared<
    leaf_detail::polymorphic_context_impl<context_type_from_handlers<H...>>>();
}
...
public:
  ...
  friend std::basic_ostream<CharT, Traits>& operator<<(
    std::basic_ostream<CharT, Traits>& os,
    verbose_diagnostic_info const& x)
  {
  ...
  : handler_argument_always_available<e_unexpected_info>
{
    ...
    error_info const& ei) noexcept
  {
    return verbose_diagnostic_info_(
      ei,
      handler_argument_traits_defaults<e_unexpected_info>::check(tup, ei),
      tup);
  }
};
}  // namespace leaf_detail
...
handler_argument_traits<void>::check(Tup const&, error_info const& ei) noexcept
{
  return ei.exception();
}
...
BOOST_LEAF_CONSTEXPR inline bool check_handler_(
  Tup& tup,
  error_info const& ei,
  leaf_detail_mp11::mp_list<A...>) noexcept
{
  ...
  handle_error_(Tup& tup, error_info const& ei, H&& h)
{
  ...
  return handler_caller<R, H>::call(
    tup, ei, std::forward<H>(h), fn_mp_args<H>{});
}
  ...
  handle_error_(Tup& tup, error_info const& ei, Car&& car, Cdr&&... cdr)
{
  if (handler_matches_any_error<fn_mp_args<Car>>::value ||
    ...
    return handler_caller<R, Car>::call(
      tup, ei, std::forward<Car>(car), fn_mp_args<Car>{});
  else
...
BOOST_LEAF_CONSTEXPR inline R handle_error_tuple_(
  ...
  Cdr&&... cdr)
{
  return handle_error_<R>(tup,
                          ei,
                          std::get<I>(std::forward<HTup>(htup))...,
                          std::forward<Cdr>(cdr)...);
}
...
{
  BOOST_LEAF_ASSERT(!is_active());
  return leaf_detail::handle_error_<R>(
    tup(), error_info(id), std::forward<H>(h)...);
}
...
{
  static_assert(
    sizeof(context_type_from_handlers<H...>) > 0,
    "When exceptions are disabled, try_catch can't fail and has no use for the "
    "handlers, but this ensures that the supplied H... types are compatible.");
...
BOOST_LEAF_CONSTEXPR BOOST_LEAF_ALWAYS_INLINE bool
cmp_value_pack(MatchType const& e, VCar car, VCdr... cdr)
{
...
template<class Enum,
         BOOST_LEAF_MATCH_ARGS(
           BOOST_LEAF_ESC(match_enum_type<condition<Enum, Enum>>),
           V1,
           V)>
...
template<class E,
         class Enum,
         BOOST_LEAF_MATCH_ARGS(
           BOOST_LEAF_ESC(match_value_enum_type<condition<E, Enum>>),
           V1,
           V)>
    ...
    operator error_id() noexcept
    {
      switch (r_.what_.kind()) {
...
#if BOOST_LEAF_CFG_CAPTURE
        {
          ...
          tls::write_uint32<leaf_detail::tls_tag_id_factory_current_id>(
            captured_id.value());
          return captured_id;
        }
#else
